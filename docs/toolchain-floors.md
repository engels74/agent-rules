...
1. **It only bites on a *fresh* lockfile.** Bun 1.4 writes v2 for a new lockfile
   but preserves v1 in an existing one. `yt-redirect` was hit because its
   lockfile was recreated when migrating off npm; the other, whose `bun.lock`
   predates the migration, stayed v1 and stayed green. Any repo that
   regenerates its lockfile inherits the break.
